    ...
    original.render = function (row, daily, key) {
      ...
      const draw = (id, actual, normal, name, color) => {
        ...
        new Chart(canvas, { type: 'line', data: { labels: rows.map(x => String(x.date || '').slice(5, 10).replace('-', '/')), datasets: [
          ...
          { label: `${name}相对常态（历史同期基准）`, data: rows.map(rel), borderColor: color, backgroundColor: color === '#8c6a3d' ? 'rgba(140,106,61,.18)' : 'rgba(76,147,198,.18)', fill: 'origin', borderWidth: 1.5, pointRadius: 0, tension: .22, yAxisID: 'yRelative' }
